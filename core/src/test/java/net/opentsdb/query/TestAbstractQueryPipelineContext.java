...
public class TestAbstractQueryPipelineContext {
    ...
    public void testFetchNext() throws Exception {
        ...
        assertEquals(3, ctx.plan().graph().nodes().size());
        assertEquals(1, ctx.plan().serializationSources().size());

        // graph: sink -> expression -> m1
        assertTrue(ctx.plan().graph().hasEdgeConnecting(ctx.plan().nodeForId("expression"),
                ctx.plan().nodeForId("m1")));
